<template>
    ...
    <v-app> 
        ...
        <v-main> 
            ...
            >          
                <v-toolbar-title class="white--text">Produtos Cadastrados: ({{ qntProdutos }})</v-toolbar-title> 
                <v-spacer />
                
                <!-- Botão para adicionar novo registro -->
                <v-col cols="auto">
                    <v-tooltip text="Novo Produto" location="top">
            ...
            <v-container fluid class="pa-4"> 
                ...
                <v-card class="pa-4" style="width: 100%; top: 60px">
                    ...
                    <v-text-field
                        ...
                        variant="outlined"
                        clearable
                        @keyup.enter="carregaListaProdutos"
                        @click:clear="carregaListaProdutos"
                    ...
                    <v-data-table-server
                        ...
                        v-model:options="opcoesTabelaProdutos"
                        :headers="headersProdutos"
                        :items="listaProdutos"
                        :loading="carregandoProdutos"
                        @update:options="salvaItensPorPagina"
                            ...
                            <v-tooltip text="Editar Produto" location="bottom">
                                ...
                                <template #activator="{ props }">
                                    ...
                                    <v-icon
                                        ...
                                        size="24" 
                                        @click="editarProduto(item.idProduto)"
...
<script setup>
    ...
    const textoBuscaProduto = ref('')
    const listaProdutos = ref([])
    const qntProdutos = ref(0)
    
    const opcoesTabelaProdutos = ref({ 
        page: 1,
        itemsPerPage: 10,
        sortBy: ['nome'],
        sortDesc: [false]
    ...
    async function carregaListaProdutos() {
        ...
        const sortArray = Array.isArray(opcoesTabelaProdutos.value?.sortBy) 
            ? opcoesTabelaProdutos.value.sortBy 
            : []
        ...
        // Extrai paginação
        const paginaAtual = opcoesTabelaProdutos.value.page ?? 1
        const itensPorPagina = localStorage.getItem('itensPorPagina') ?? 10
        ...
        try{
            ...
            if (carregandoProdutos.value == false ){

                // Seta carregando
                carregandoProdutos.value = true

                // Chama a API para carregar a lista de empresas
                const { data } = await axios.get(`${API_BASE_URL}/administracao/carregaListaProdutos`,{
                    params: { 
                        textoBuscaProduto: textoBuscaProduto.value,
    ...
    function salvaItensPorPagina(){

        localStorage.setItem('itensPorPagina', opcoesTabelaProdutos.value.itemsPerPage)
    ...
    function criaNovoProduto() {        
        router.push({ name: 'CadastroProdutos', query: { idProduto: 0 } })        
    }

    // Esta função chama o cadastro de empresas para editar a empresa apontada por idProduto
    function editarProduto(idProduto) {
        router.push({ name: 'CadastroProdutos', query: { idProduto: idProduto } })
    }
