<template>
    ...
    <v-app> 
        ...
        <v-main> 
            ...
            >          
                <v-toolbar-title class="white--text">Mensagens Cadastradas: ({{ qntMensagens }})</v-toolbar-title> 
                <v-spacer />
                
                <!-- Botão para adicionar novo registro -->
                <v-col cols="auto">
                    <v-tooltip text="Nova Mensagem" location="top">
            ...
            <v-container fluid class="pa-4"> 
                ...
                <v-card class="pa-4" style="width: 100%; top: 60px">
                    ...
                    <!-- Campo de busca -->
                    <v-text-field
                        v-model="textoBuscaMensagem"
                        label="Pesquisar"
                        placeholder="Digite sua busca..."
                        prepend-inner-icon="mdi-magnify"
                        variant="outlined"
                        clearable
                        @keyup.enter="carregaListaMensagens"
                        @click:clear="carregaListaMensagens"
                    ...
                    <!-- Tabela de dados -->
                    <v-data-table-server
                        item-value="idMensagem"
                        v-model:options="opcoesTabelaMensagens"
                        :headers="headersMensagens"
                        :items="listaMensagens"
                        :loading="carregandoMensagens"
                        @update:options="carregaListaMensagens"
                        class="elevation-2 rounded-lg tabela-pequena"
                        density="compact"
                        :items-length="totalMensagens"
                        :footer-props="{
                    ...
                    >                
                        <template v-slot:item.funcoes="{ item }">
                            ...
                            <v-tooltip text="Editar Mensagem" location="bottom">
                                ...
                                <template #activator="{ props }">
                                    ...
                                    <v-icon
                                        ...
                                        size="24" 
                                        @click="editaMensagem(item.idMensagem)"
...
<script setup>
    ...
    const textoBuscaMensagem = ref('')
    const listaMensagens = ref([])
    const qntMensagens = ref(0)
    
    const opcoesTabelaMensagens = ref({ 
        page: 1,
        itemsPerPage: 10,
        sortBy: ['nome'],
        sortDesc: [false]
    ...
    const headersMensagens = [
        ...
        { title: 'Código', value: 'idMensagem', sortable: true },
        { title: 'Assunto', value: 'assunto', sortable: true },
        { title: 'Provedor', value: 'provedor', sortable: true },
        { title: 'Condição', value: 'ativo', sortable: true }        
    ]

    // Função que será chamada sempre que a tabela mudar de página, ordenação ou filtro
    async function carregaListaMensagens() {
        ...
        const sortArray = Array.isArray(opcoesTabelaMensagens.value?.sortBy) 
            ? opcoesTabelaMensagens.value.sortBy 
            : []
        ...
        // Extrai paginação
        const paginaAtual = opcoesTabelaMensagens.value.page ?? 1
        const itensPorPagina = opcoesTabelaMensagens.value.itemsPerPage ?? 10
        
        // Limpa lista de empresas
        listaMensagens.value = []

        // Reseta valor total de empresas
        totalMensagens.value = 0

        try{

            // Verifica se já está carregando dados da empresa
            if (carregandoMensagens.value == false ){

                // Seta carregando
                carregandoMensagens.value = true

                // Chama a API para carregar a lista de empresas
                const { data } = await axios.get(`${API_BASE_URL}/administracao/carregaListaMensagens`,{
                    params: { 
                        textoBuscaMensagem: textoBuscaMensagem.value,
                ...
                qntMensagens.value = retorno.length

                // Atualiza a lista de empresas e o total
                listaMensagens.value = retorno
                totalMensagens.value = data.total            
            }
    ...
    // Se o campo foi limpo, recarrega a lista mensagens
    function atualizaParamentrosBusca(novoValor) {
        if (!novoValor) {
            carregaListaMensagens()
        }
    }

    // Esta função chama o cadastro de empresas para criar uma nova empresa
    function criaMensagem() {        
        router.push({ name: 'CadastroMensagens', query: { idMensagem: 0 } })        
    }

    // Esta função chama o cadastro de empresas para editar a empresa apontada por idMensagem
    function editaMensagem(idMensagem) {
        router.push({ name: 'CadastroMensagens', query: { idMensagem: idMensagem } })
    }
