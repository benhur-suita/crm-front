<template>
    ...
    <v-app> 
        ...
        <v-main> 
            ...
            >        
                <v-toolbar-title class="white--text">Tipos Chamados Cadastrados: ({{ qntTiposChamados }})</v-toolbar-title> 
                <v-spacer />
                
                <!-- Botão para adicionar novo registro -->
                <v-col cols="auto">
                    <v-tooltip text="Novo Tipo Chamado" location="top">
            ...
            <v-container fluid class="pa-4"> 
                ...
                <v-card class="pa-4" style="width: 100%; top: 60px">
                    ...
                    <!-- Campo de busca -->
                    <v-text-field
                        v-model="textoBuscaTipoChamado"
                        label="Pesquisar"
                        placeholder="Digite sua busca..."
                        prepend-inner-icon="mdi-magnify"
                        variant="outlined"
                        clearable
                        @keyup.enter="carregaListaTiposChamados"
                        @click:clear="carregaListaTiposChamados"
                    ...
                    <v-data-table-server
                        ...
                        v-model:options="opcoesTabelaTiposChamados"
                        :headers="headersTiposChamados"
                        :items="listaTiposChamados"
                        :loading="carregandoTiposChamados"
                        @update:options="carregaListaTiposChamados"
                        class="elevation-2 rounded-lg tabela-pequena"
                        density="compact"
                        :items-length="totalTiposChamados"
                        :footer-props="{
                    ...
                    >                
                        <template v-slot:item.funcoes="{ item }">
                            ...
                            <v-tooltip text="Editar Tipo Chamado" location="bottom">
                                ...
                                <template #activator="{ props }">
                                    ...
                                    <v-icon
                                        ...
                                        size="24" 
                                        @click="editaTipoChamado(item.idTipoChamado)"
...
<script setup>
    ...
    const totalTiposChamados = ref(0)
    const carregandoTiposChamados = ref(false)    
    const textoBuscaTipoChamado = ref('')
    const listaTiposChamados = ref([])
    const qntTiposChamados = ref(0)
    
    const opcoesTabelaTiposChamados = ref({ 
        page: 1,
        itemsPerPage: 10,
        sortBy: ['nome'],
        sortDesc: [false]
    ...
    async function carregaListaTiposChamados() {
        ...
        const sortArray = Array.isArray(opcoesTabelaTiposChamados.value?.sortBy) 
            ? opcoesTabelaTiposChamados.value.sortBy 
            : []
        ...
        // Extrai paginação
        const paginaAtual = opcoesTabelaTiposChamados.value.page ?? 1
        const itensPorPagina = opcoesTabelaTiposChamados.value.itemsPerPage ?? 10
        
        // Limpa lista de empresas
        listaTiposChamados.value = []

        // Reseta valor total de empresas
        totalTiposChamados.value = 0

        try{

            // Verifica se já está carregando dados da empresa
            if (carregandoTiposChamados.value == false ){

                // Seta carregando
                carregandoTiposChamados.value = true

                // Chama a API para carregar a lista de empresas
                const { data } = await axios.get(`${API_BASE_URL}/administracao/carregaListaTiposChamado`,{
                    params: { 
                        textoBuscaTipoChamado: textoBuscaTipoChamado.value,
                ...
                qntTiposChamados.value = retorno.length

                // Atualiza a lista de empresas e o total
                listaTiposChamados.value = retorno
                totalTiposChamados.value = data.total            
            }

        } catch(erro){
            
            // Em caso de erro desliga indicador de carregamento e mostra o erro
            carregandoTiposChamados.value = false
    ...
    function atualizaParamentrosBusca(novoValor) {
        if (!novoValor) {
            carregaListaTiposChamados()
        }
    }

    // Esta função chama o cadastro de empresas para criar uma nova empresa
    function criaNovoTipoChamado() {        
        router.push({ name: 'CadastroTiposChamados', query: { idTipoChamado: 0 } })        
    }

    // Esta função chama o cadastro de empresas para editar a empresa apontada por idEmpresa
    function editaTipoChamado(idTipoChamado) {
        router.push({ name: 'CadastroTiposChamados', query: { idTipoChamado: idTipoChamado } })
    }
