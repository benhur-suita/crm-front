<template>
    ...
    <v-app> 
        ...
        <v-main> 
            ...
            >          
                <v-toolbar-title class="white--text">Ramos Atividade Cadastrados: ({{ qntRamosAtividade }})</v-toolbar-title> 
                <v-spacer />
                
                <!-- Botão para adicionar novo registro -->
                <v-col cols="auto">
                    <v-tooltip text="Novo Ramo Atividade" location="top">
            ...
            <v-container fluid class="pa-4"> 
                ...
                <v-card class="pa-4" style="width: 100%; top: 60px">
                    ...
                    <!-- Campo de busca -->
                    <v-text-field
                        v-model="textoBucaRamoAtividade"
                        label="Pesquisar"
                        placeholder="Digite sua busca..."
                        prepend-inner-icon="mdi-magnify"
                        variant="outlined"
                        clearable
                        @keyup.enter="carregaListaRamosAtividade"
                        @click:clear="carregaListaRamosAtividade"
                        @update:modelValue="carregaListaRamosAtividade"
                    ...
                    <v-data-table-server
                        ...
                        v-model:options="opcoesTabelaRamosAtividade"
                        :headers="headersRamosAtividade"
                        :items="listaRamosAtividade"
                        :loading="carregandoRamoAtividade"
                        @update:options="carregaListaRamosAtividade"
                        class="elevation-2 rounded-lg tabela-pequena"
                        density="compact"
                        :items-length="totalRamosAtividade"
                        :footer-props="{
                    ...
                    >                
                        <template v-slot:item.funcoes="{ item }">
                            ...
                            <v-tooltip text="Editar Ramo Atividade" location="bottom">
                                ...
                                <template #activator="{ props }">
                                    ...
                                    <v-icon
                                        ...
                                        size="24" 
                                        @click="editaRamoAtividade(item.idRamoAtividade)"
...
<script setup>
    ...
    const totalRamosAtividade = ref(0)
    const carregandoRamoAtividade = ref(false)    
    const textoBucaRamoAtividade = ref('')
    const listaRamosAtividade = ref([])
    const qntRamosAtividade = ref(0)
    
    const opcoesTabelaRamosAtividade = ref({ 
        page: 1,
        itemsPerPage: 10,
        sortBy: ['nome'],
        sortDesc: [false]
    ...
    const headersRamosAtividade = [
        { title: 'Funções', value: 'funcoes', sortable: false },
        { title: 'Código', value: 'idRamoAtividade', sortable: true },
        { title: 'Ramo Atividade', value: 'nome', sortable: true },
        { title: 'Condição', value: 'ativo', sortable: true }        
    ]

    // Função que será chamada sempre que a tabela mudar de página, ordenação ou filtro
    async function carregaListaRamosAtividade() {
        ...
        const sortArray = Array.isArray(opcoesTabelaRamosAtividade.value?.sortBy) 
            ? opcoesTabelaRamosAtividade.value.sortBy 
            : []
        ...
        // Extrai paginação
        const paginaAtual = opcoesTabelaRamosAtividade.value.page ?? 1
        const itensPorPagina = opcoesTabelaRamosAtividade.value.itemsPerPage ?? 10
        
        // Limpa lista de empresas
        listaRamosAtividade.value = []

        // Reseta valor total de empresas
        totalRamosAtividade.value = 0

        try{

            // Verifica se já está carregando dados da empresa
            if (carregandoRamoAtividade.value == false ){

                // Seta carregando
                carregandoRamoAtividade.value = true

                // Chama a API para carregar a lista de empresas
                const { data } = await axios.get(`${API_BASE_URL}/administracao/carregaListaRamosAtividade`,{
                    params: { 
                        textoBucaRamoAtividade: textoBucaRamoAtividade.value,
                ...
                qntRamosAtividade.value = retorno.length

                // Atualiza a lista de empresas e o total
                listaRamosAtividade.value = retorno
                totalRamosAtividade.value = data.total            
            }

        } catch(erro){
            
            // Em caso de erro desliga indicador de carregamento e mostra o erro
            carregandoRamoAtividade.value = false
    ...
    function atualizaParamentrosBusca(novoValor) {
        if (!novoValor) {
            carregaListaRamosAtividade()
        }
    }

    // Esta função chama o cadastro de empresas para criar uma nova empresa
    function criaNovoRamoAtividade() {        
        router.push({ name: 'CadastroRamosAtividade', query: { idRamoAtividade: 0 } })        
    }

    // Esta função chama o cadastro de empresas para editar a empresa apontada por idEmpresa
    function editaRamoAtividade(idRamoAtividade) {
        router.push({ name: 'CadastroRamosAtividade', query: { idRamoAtividade: idRamoAtividade } })
    }

    // Função para forçar recarga quando necessário
    function recarregarRamosAtividade() {
        opcoesTabelaRamosAtividade.value.page = 1
        carregaListaRamosAtividade()
    }
