<template>
    ...
    <v-app> 
        ...
        <v-main> 
            ...
            >          
                <v-toolbar-title class="white--text">E-mail Cadastrados: ({{ qntEmail }})</v-toolbar-title> 
                <v-spacer />
                
                <!-- Botão para adicionar novo registro -->
                <v-col cols="auto">
                    <v-tooltip text="Novo E-mail" location="top">
            ...
            <v-container fluid class="pa-4"> 
                ...
                <v-card class="pa-4" style="width: 100%; top: 60px">
                    ...
                    <!-- Campo de busca -->
                    <v-text-field
                        v-model="textoBuscaEmail"
                        label="Pesquisar"
                        placeholder="Digite sua busca..."
                        prepend-inner-icon="mdi-magnify"
                        variant="outlined"
                        clearable
                        @keyup.enter="carregandoListaEmail"
                        @click:clear="carregandoListaEmail"
                    ...
                    <v-data-table-server
                        item-value="idEmail"
                        v-model:options="opcoesTabelaEmail"
                        :headers="headersEmail"
                        :items="listaEmail"
                        :loading="carregandoEmail"
                        @update:options="carregandoListaEmail"
                        class="elevation-2 rounded-lg tabela-pequena"
                        density="compact"
                        :items-length="totalEmail"
                        :footer-props="{
                    ...
                    >                
                        <template v-slot:item.funcoes="{ item }">
                            ...
                            <v-tooltip text="Editar E-mail" location="bottom">
...
<script setup>
    ...
    const textoBuscaEmail = ref('')
    const listaEmail = ref([])
    const qntEmail = ref(0)
    
    const opcoesTabelaEmail = ref({ 
        page: 1,
        itemsPerPage: 10,
        sortBy: ['nome'],
        sortDesc: [false]
    ...
    const headersEmail = [
        ...
        { title: 'Código', value: 'idEmail', sortable: true },
        { title: 'Nome', value: 'nome', sortable: true },
        { title: 'Email Fonte', value: 'email_fonte', sortable: true },
        { title: 'Servidor', value: 'servidor', sortable: true },
        { title: 'Porta', value: 'porta', sortable: true },
        { title: 'Usuario', value: 'usuario', sortable: true },
        { title: 'Condição', value: 'ativo', sortable: true }        
    ]

    // Função que será chamada sempre que a tabela mudar de página, ordenação ou filtro
    async function carregandoListaEmail() {
        ...
        const sortArray = Array.isArray(opcoesTabelaEmail.value?.sortBy) 
            ? opcoesTabelaEmail.value.sortBy 
            : []
        ...
        // Extrai paginação
        const paginaAtual = opcoesTabelaEmail.value.page ?? 1
        const itensPorPagina = opcoesTabelaEmail.value.itemsPerPage ?? 10
        ...
        try{
            ...
            if (carregandoEmail.value == false ){

                // Seta carregando
                carregandoEmail.value = true

                // Chama a API para carregar a lista de empresas
                const { data } = await axios.get(`${API_BASE_URL}/operacao/carregaListaEmail`,{
                    params: { 
                        textoBuscaEmail: textoBuscaEmail.value,
                ...
                qntEmail.value = retorno.length

                // Atualiza a lista de empresas e o total
                listaEmail.value = retorno
                totalEmail.value = data.total            
            }
    ...
    function atualizaParamentrosBusca(novoValor) {
        if (!novoValor) {
            carregandoListaEmail()
        }
    }

    // Esta função chama o cadastro de empresas para criar uma nova empresa
    function criaNovoEmail() {        
        router.push({ name: 'CadastroEmail', query: { idEmail: 0 } })        
    }

    // Esta função chama o cadastro de empresas para editar a empresa apontada por idEmail
    function editaEmail(idEmail) {
        router.push({ name: 'CadastroEmail', query: { idEmail: idEmail } })
    }
